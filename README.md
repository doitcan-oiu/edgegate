...
**自定义服务商的推理请求全部经过 Cloudflare AI Gateway。** 供应商 API Key 默认由本程序加密保存在 D1，无需配置 BYOK 或 Secrets Store。本程序调用 Cloudflare API 管理服务商，页面加载、筛选、分页及刷新时由 Worker 直接读取 Cloudflare REST 日志与 GraphQL 统计；费用采用 Cloudflare 数据，D1 / KV 不缓存这些数据。
...
flowchart LR
    ...
    AIG --> Custom[自定义服务商]
    Worker -->|服务商管理 / 日志与统计直读| API[Cloudflare REST + GraphQL API]
    Worker --> D1[(D1 · 业务配置 / 配额 / 错误追踪)]
    Worker --> KV[(KV · 管理员会话)]
```
...
| 日志列表与详情 | 每次页面请求直接读取 Cloudflare REST API |
| 总览、趋势、模型分布 | 每次页面请求直接读取 Cloudflare GraphQL `aiGatewayRequestsAdaptiveGroups` |
...
- Anthropic 客户端调用 OpenAI 服务商时，流式请求会向上游请求 `include_usage`。Anthropic 的初始流式计数从 0 开始，结束事件更新为上游报告值；若上游缺少必要用量、缺少结束事件或出现无法表示的内容，转换会报错。不会自行补算用量或账单；页面展示 Cloudflare 实际报告的记录。
...
2. 选择一个或多个标签：可使用匹配任意所选标签（OR）的渠道路由，以及管理员明确加入这些标签的跨标签路由。无标签渠道也可作为明确配置的跨标签上游。标签区分大小写，并去重。
3. `/v1/models` 返回符合标签范围、拥有已启用且已配置渠道路由的公开模型的**并集**，再与 Key 的可选模型白名单取交集。
4. 标签限制同时应用于 Chat Completions、Responses、Messages 和每次故障转移；跨标签访问只开放明确配置的模型路由，不会开放目标渠道的其他模型，也不能通过请求参数扩大权限。
5. 继承渠道标签的路由随服务商标签变化；跨标签路由固定归属添加时选定的标签，修改目标渠道标签不会改变其归属。已有 Key 无需重新签发，权限查询不使用 KV 缓存。
...
「模型与路由」通过标签按钮切换管理范围，默认选中第一个可用标签；模型目录、路由数量和列表按路由的有效标签筛选。添加或编辑路由时可选择所有渠道，下拉框标明渠道标签，当前标签的渠道排在前面。相同模型 ID 在多个标签下复用全局模型。

例如：选择标签 `A` → 模型 `gpt5.6-sol` →「添加路由」→ 选择标签 `B` 的渠道，上游模型填 `gpt5.6-sol`。新路由归属 `A`，只授权 `A` 的令牌即可调用它，并按设置的优先级与权重参与分配和故障转移。`B` 下的原有同名路由保留独立设置；修改或删除 `A` 的跨标签路由不会修改 `B` 的原有路由。

旧路由、自动路由和同标签新建路由继续继承渠道标签，一个渠道有多个标签时，这些路由仍在多个视图中共享。跨标签路由用 `scope_tag` 保存固定归属（空字符串表示继承渠道标签），即使原渠道标签被移除，仍可通过该标签管理路由及创建 Key。将继承标签的路由改到其他标签渠道时，会固定到当前管理标签；以后编辑该路由保留固定归属。无标签渠道的继承路由可通过「未打标签」管理，该视图仍只允许选择无标签渠道。

管理 API 的 `/models`、`/routes` 支持 `?tag=标签` 或 `?untagged=1`。`POST /routes?tag=A` 的目标渠道不含 `A` 时创建固定归属 `A` 的跨标签路由；相同模型、渠道和上游模型可在不同归属下独立配置。带范围的编辑、删除操作校验原路由的有效标签，不能通过 `A` 视图直接修改 `B` 的路由。模型全局写入不接受标签范围。

修改清单只删除该清单管理的自动路由，保留手工配置的模型别名；编辑自动路由后，该路由转为手工管理。没有可用路由的模型不会出现在 `/v1/models` 中。全局模型元数据仍可通过不带标签范围的管理 API 维护，页面只管理所选标签的路由。

Playground 同样通过标签按钮选择调试范围，只列出该范围中已配置、已启用渠道的可用模型，包含明确加入该标签的跨标签路由。请求、重新生成及故障重试遵守相同规则；切换标签保留对话和草稿，每条回复标明发起时的标签。调用代码仍使用应用 API Key 的标签授权：仅授权当前标签可复现相同渠道范围，API Key 暂不支持仅授权未打标签渠道。

升级时执行 `npm run db:migrate`（生产使用 `npm run db:migrate:remote`），应用全部未执行的迁移。`0011_cross_tag_routes.sql` 新增路由归属并扩展唯一约束，保留原有路由 ID、优先级、权重、状态与自动管理标记；现有路由继续继承渠道标签，已有 Key 的权限不变。

## 日志与分析

数据来源是 Cloudflare AI Gateway。打开页面、刷新、切换统计范围、日志筛选与分页时，Worker 都会直接读取 Cloudflare，浏览器不会持有 Cloudflare Token。没有后台数据同步、历史回补或本地保留期设置。

| 内容 | 读取方式 |
| --- | --- |
| 日志列表、筛选与分页 | Cloudflare REST API，按 `created_at` 降序读取 |
| 日志详情 | 每次打开直接读取 Cloudflare REST API，并关联 D1 中的上游错误追踪 |
| 总览、趋势、模型分布 | Cloudflare GraphQL，统计整个网关的过去 24 小时或 7 天 |
| 渠道可用率 | 本次请求分页读取 Cloudflare 最近 1 小时日志，并按渠道 ID 汇总 |
| 数据保留 | 由 Cloudflare AI Gateway 的日志设置决定 |

- Cloudflare REST 读取及 GraphQL 查询遇到 HTTP 5xx 时，最多额外重试 2 次（共最多 3 次请求）。重试后仍失败会返回错误，页面提供重试 / 刷新操作，不将失败显示为零流量或无日志。
- D1 不保存 Cloudflare 日志、统计快照、同步状态或数据保留设置；KV 不缓存 Cloudflare 数据或分析 Schema，只保存管理员会话。D1 继续保存业务配置、配额和上游错误追踪。
- 日志总条数采用 Cloudflare 返回值，未报告时显示「—」；是否能翻到下一页由响应的 `has_more` 决定。分页过程中 Cloudflare 新增或删除记录可能改变页面位置，日志保留与可见范围以 Cloudflare 当前返回为准。
- 渠道卡片显示最近 1 小时的分段可用率，共 30 块，每块代表 2 分钟。按日志中的渠道 ID 汇总，同名渠道互不混用；缓存命中、缺少成功状态或渠道 ID 的记录不参与统计。重试按各次上游尝试计算，灰色表示本次样本中无有效记录。这是实际请求样本的成功率，不是主动探测在线率。
- 可用率响应包含统计时间范围、读取时间、有效唯一样本数和 `coverage`。达到本次读取预算、分页未前进或后续页读取失败时，`coverage: "partial"`，页面标明「样本不完整」及原因；首个 Cloudflare 请求失败会显示读取错误。`complete` 仅表示本次已读完 Cloudflare 可返回的时间范围，不能恢复未采集、延迟出现或已删除的日志。
- 统计来自 GraphQL 的整个网关聚合，不以日志样本补算请求量、用量和费用。图表按本次返回的起止时间绘制。缺失的状态、Token 或费用显示「未报告」或 `—`。
...
在「模型与路由」为应用模型添加路由组，每组可设置名称、优先级、权重、组内分配策略及启停。路由表单可选择所属组；编辑已有路由即可移组。组设置属于全局模型，会对该模型的所有标签生效；令牌权限仍按每条路由的标签过滤，分组本身不授予额外访问权限。空组在各标签视图可见，非空组只在包含其可见路由的标签视图中展示。删除组前必须先移出或删除所有标签下的组内路由。

在「网关设置 → 程序设置」配置**组间**负载均衡、重试与错误展示。设置保存在 D1，保存后对新请求生效，进行中的请求使用开始时的配置。

- 组间分配：先选择较低组优先级，同优先级按全局「随机」或「按照权重」选择组。组权重与组内路由数量、路由权重无关。
- 组内分配：先选择较低路由优先级，同级按该组设置选择「随机」（各路由等概率）、「按照权重」（路由权重）或「轮询」（按稳定路由 ID 顺序循环）。当前组的可用路由耗尽后才切换到下一组。每次请求仍只尝试同一渠道一次，不因跨组或多个上游模型重复进入该渠道；同渠道重试由单独的重试设置控制。
- 轮询：D1 原子游标支持并发请求与不同 Worker 实例；每组、路由优先级和实际授权且协议兼容的路由集合分别计数。未进入的后备组不推进游标；可用集合变化时使用对应集合的独立游标。
- 旧路由及新建时未选组的路由显示在「默认组」：组优先级 P0、组权重 1，组内继续沿用原全局分配策略（随机时按渠道等概率），保持原配置行为。要独立设置组内策略，请将这些路由移入自建组。相同模型、渠道、上游模型及标签归属的路由只能保存一条；换组请编辑原路由。
...
例如 `claude-opus-4-6` 下建立「主力组」P0 和「备用组」P1，主力组内放三条 P0 路由并将组内策略设为随机。全局设为按照权重时，请求仍先使用主力组，并在三条路由中等概率选择；需要故障转移时先尝试主力组剩余路由，再进入备用组。若希望两个组按权重分担正常流量，应把两个组设为相同优先级。重试次数上限始终生效，达到上限就停止，不保证一定进入备用组。

`GET /api/models` 返回各模型的 `groups`（含所有标签的成员总数 `route_count`）及路由的 `group_id`。`POST /api/route-groups` 创建组，传 `create_model: true` 可同时创建新模型；`PUT /api/route-groups/{id}` 更新组，不能改变其所属模型；`DELETE` 仅删除空组。组接口不接受标签范围参数。路由写入显式传 `group_id: null` 可移回默认组；旧客户端编辑时省略该字段会保留现有分组，并发分组变更会返回冲突。

升级需应用 `0012_route_groups.sql`，新增路由组、路由分组引用及轮询游标表。原路由 ID、标签权限、优先级、权重、凭据和唯一约束保持不变。
...
worker/
  ...
  observability.ts       日志、统计与渠道可用率直读接口
  cloudflare-observability.ts Cloudflare 日志与 GraphQL 直接读取
