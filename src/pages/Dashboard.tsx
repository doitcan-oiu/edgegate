...
function TrafficChart({ stats }: { stats: Stats | null }) {
  if (!stats) return <div className="chart-offline"><Activity size={20} aria-hidden="true" /><strong>等待网关数据</strong><p>完成 AI Gateway 配置后，查看真实请求趋势。</p><a href="#settings" className="text-link">接入 Cloudflare<ArrowRight size={15} /></a></div>;
  ...
  const points = bins.map((bin, i) => `${45 + i * 670 / (count - 1)},${145 - bin.requests / max * 120}`).join(' ');
  const errors = bins.map((bin, i) => `${45 + i * 670 / (count - 1)},${145 - bin.errors / max * 120}`).join(' ');
  return <div className="traffic-chart"><svg viewBox="0 0 740 190" role="img" aria-label={`请求量趋势，共 ${stats.summary.requests} 次请求`}>
    {[0, 1, 2, 3, 4].map(i => <g key={i}><line x1="45" x2="715" y1={25 + i * 30} y2={25 + i * 30} stroke="var(--border-subtle)" strokeDasharray="3 5" /><text x="30" y={29 + i * 30} textAnchor="end">{compact(max - max * i / 4)}</text></g>)}
    <polygon points={`45,145 ${points} 715,145`} fill="var(--brand-pale-green)" /><polyline points={points} fill="none" stroke="var(--brand-green)" strokeWidth="2.5" strokeLinejoin="round" />
    {stats.summary.successes !== null && <polyline points={errors} fill="none" stroke="var(--brand-coral)" strokeWidth="1.5" strokeDasharray="4 3" />}
    {bins.filter((_, i) => i % (hourly ? 6 : 1) === 0).map(bin => <text key={bin.date.toISOString()} x={45 + bins.indexOf(bin) * 670 / (count - 1)} y="178" textAnchor="middle">{hourly ? bin.date.toLocaleTimeString('zh-CN', { hour: '2-digit', minute: '2-digit', hour12: false }) : `${bin.date.getMonth() + 1}/${bin.date.getDate()}`}</text>)}
...
export function Dashboard() {
  ...
  const metrics = [
    { label: '请求成功率', value: summary?.requests && summary.successes !== null ? `${(summary.successes / summary.requests * 100).toFixed(1)}%` : '—', note: summary?.successes != null ? `${number(summary.successes)} 次成功响应` : '暂无分析数据' },
    { label: '缓存命中率', value: summary?.requests && summary.cache_hits !== null ? `${(summary.cache_hits / summary.requests * 100).toFixed(1)}%` : '—', note: 'Cloudflare 网关缓存' },
