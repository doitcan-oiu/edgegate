...
export function Logs() {
  ...
  return <><PageTitle title="请求日志" description="追踪每一次上游调用，检查响应状态与请求详情。" action={<Button variant="secondary" onClick={() => setTracing(true)}><Search size={16} />请求追踪</Button>} /><ErrorBox message={logs.error} />{tracing && <RequestTrace onClose={() => setTracing(false)} />}
    <ObservabilitySync jobs={['logs:head']} /><div className="log-summary"><span><strong>{logs.data?.total ?? '—'}</strong>本地匹配日志</span><span className="log-success-count"><strong>{logs.data?.data.filter(log => log.success).length ?? '—'}</strong>本页成功</span><span className="log-error-count"><strong>{logs.data?.data.filter(log => !log.success).length ?? '—'}</strong>本页失败</span><span className="log-scope"><Cloud size={16} />Cloudflare AI Gateway</span></div>
    ...
    {logs.loading ? <Loading /> : logs.data?.data.length ? <LogTable logs={logs.data.data} onSelect={setSelected} /> : !logs.error && <Empty title={logs.data?.sync.last_success_at ? '没有匹配的已同步日志' : '等待首次日志同步'} description="首次同步、历史回补或新请求入库需要一些时间；可查看同步状态或手动安排同步。" />}
    ...
    <p className="page-footnote">列表仅包含本地已同步且在保留期内的日志，包含当前 Gateway 的其他客户端调用。一次 EdgeGate 请求若尝试多个上游，可能对应多条 Cloudflare 日志，可通过详情中的 EdgeGate 请求 ID 关联。Worker 提前拒绝的鉴权或配额请求不会进入 AI Gateway。</p>{selected && <LogDetail log={selected} onClose={() => setSelected(null)} />}</>;
}
