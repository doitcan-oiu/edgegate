...
function RouteForm({ modelId: initialModelId, route, channels, models, scope, onClose, onSaved }: { modelId: string; route: Route | null; channels: Channel[]; models: Model[]; scope: ModelScope; onClose: () => void; onSaved: (modelId: string) => void }) {
  ...
  return <Modal title={route ? '编辑路由' : '添加路由'} description={`${initialModelId ? `为 ${modelId} 配置路由；` : ''}仅管理${scope.kind === 'tag' ? `标签 ${scope.tag}` : '未打标签渠道'}的路由。`} onClose={onClose}><form onSubmit={submit} className="form-stack"><FormSection number="01" title="上游目标">{!initialModelId && <Field label="应用模型 ID" hint="输入新 ID，保存时创建应用模型并绑定当前标签的渠道；选择已有 ID 则只添加路由。">
...
export function Models() {
  ...
  const canAddRoute = ready && !!scope && !!scopedChannels.length;
  ...
  return <div className="models-page">
    ...
    <div className="model-workspace surface">
      ...
      </> : <Empty title={search ? '没有匹配的模型' : scope ? '当前标签下还没有模型路由' : '请先添加渠道'} description={search ? '试试其他模型 ID 或描述。' : scope ? '输入新模型 ID 或选择已有模型，然后绑定当前标签的渠道。' : '在渠道管理中添加渠道并设置标签，然后管理对应的模型路由。'} action={!search && (canAddRoute ? <Button variant="secondary" onClick={() => scope && setRouteEdit({ modelId: '', route: null, scope })}><Plus size={16} />添加模型路由</Button> : <Button variant="secondary" onClick={() => { window.location.hash = 'channels'; }}>前往渠道管理</Button>)} />}</section>
    </div>
    {routeEdit && <RouteForm {...routeEdit} models={models.data || []} channels={(channels.data || []).filter(channel => matchesRouteScope(channel.tags, routeEdit.scope))} onSaved={modelId => { setSelectedId(modelId); setSearch(''); }} onClose={() => setRouteEdit(null)} />}
