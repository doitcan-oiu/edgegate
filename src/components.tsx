...
export function ErrorBox({ message, onRetry }: { message?: string; onRetry?: () => void }) {
  return message ? <div className="error-box" role="alert"><AlertCircle size={17} /><span>{message}</span>{onRetry && <Button type="button" variant="ghost" onClick={onRetry}>重试</Button>}</div> : null;
}
