  ...
  return <RefreshContext.Provider value={{ version, refresh }}><ToastContext.Provider value={setToast}>
    <div className="console-shell">
      <header className="console-header"><div className="header-inner">
        <a href="#overview" aria-label="EdgeGate 总览"><Logo /></a><div className="workspace-identity"><span>默认工作空间</span></div>
        <div className="header-actions"><span className="session-label">管理员会话</span><Button variant="ghost" className="icon-btn" aria-label="刷新数据" onClick={refresh}><RefreshCw size={16} /></Button><Button variant="secondary" className="logout-button" aria-label="退出登录" onClick={logout}><LogOut size={16} /><span>退出</span></Button><Button variant="ghost" className="mobile-menu icon-btn" onClick={() => setMobile(!mobile)} aria-label="展开导航" aria-expanded={mobile} aria-controls="gateway-navigation">{mobile ? <X size={20} /> : <Menu size={20} />}</Button></div>
      </div><div className={`navigation-bar ${mobile ? 'navigation-open' : ''}`}><nav id="gateway-navigation" aria-label="主导航">{navigation.map(({ id, icon: Icon, label }) => <a key={id} href={`#${id}`} className={`nav-item ${current.id === id ? 'active' : ''}`} aria-current={current.id === id ? 'page' : undefined}><Icon size={16} /><span>{label}</span></a>)}</nav></div></header>
